<script lang="ts">
	...
	// Setting default model to index set as const
	onMount(async () => {
		// console.log("Setting default model to index set as const: ", $models[$defaultModelIndex].id);
		// selectedModels.push($models[$defaultModelIndex].id);
		// settings.set({ ...$settings, models: selectedModels });
		// localStorage.setItem('settings', JSON.stringify($settings));
	});
...
{#if $user.role === 'admin' }
	...
	<div class="flex flex-col my-2">
		{#each selectedModels as selectedModel, selectedModelIdx}
			<div class="flex">
				...
				{#if selectedModelIdx === 0}
					<button
						class=" self-center dark:hover:text-gray-300 acn-display-none"
	...
	<div class="text-left mt-1.5 text-xs text-gray-500 acn-default-model-btn ">
